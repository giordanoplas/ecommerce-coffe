...
export default function Page() {
    ...
    //const [setFilterTaste] = useState('');

    const filteredProducts = result !== null && !loading && (
        filterOrigin === '' ? result : result.filter((product: ProductType) => product.origin === filterOrigin)
        //filterTaste === '' ? result : result.filter((product: ProductType) => product.taste === filterTaste)
    )
            ...
            <div className="sm:flex sm:justify-between">
                ...
                <div className="grid gap-5 mt-8 sm:grid-cols-2 md:grid-cols-3 md:gap-10">
                    ...
                    {filteredProducts !== null && !loading && filteredProducts.length === 0 && (
                        <p className="text-red-500 font-bold">No hay productos con este filtro.</p>
                    )}
                </div>
            </div>
        </div>
